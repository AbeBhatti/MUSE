...
type DrumSound = "kick" | "snare" | "hat" | "clap";

type DrumPattern = {
  type: "drums";
  grid: boolean[][]; // 4 rows x 16 steps
};

type MelodyPattern = {
  type: "melody";
  instrument: "bass" | "synth";
  // NEW: Grid is now 6 rows (C,D,E,F,G,A)
  grid: boolean[][]; // 6 rows x 16 steps
};

type PianoNote = {
  id: string;
  note: number; // MIDI num
...
type Pattern = {
  ...
  data: DrumPattern | MelodyPattern | PianoPattern;
};
...
type TimelineClip = {
  ...
  patternId: string;
  instrument: InstrumentId;
  startBar: number; // integer bar index
  bars: number; // integer number of bars (loops)
};

// This is what's stored on the clipboard (a clip without its ID)
type ClipboardClip = Omit<TimelineClip, 'id'>;

type View =
  | { type: "library" }
  | { type: "sequencer"; instrument: InstrumentId };
...
class AudioEngine {
  ...
  startTime: number = 0; // ctx.currentTime
  startBeat: number = 0;
  current16th: number = 0;
  
  patternsRef: React.MutableRefObject<Pattern[]>;
  clipsRef: React.MutableRefObject<TimelineClip[]>;
  numBarsRef: React.MutableRefObject<number>;
  ...
  private playNote(time: number, freq: number, duration: number, instrument: "bass" | "synth" | "piano") {
    // ... existing sound synthesis code ...
    if (!this.ctx) return;
    const osc = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
  ...
  async playImmediateNote(instrument: InstrumentId, note: number | DrumSound) {
  ...
  private beatsToSec(beats: number): number {
  ...
  private scheduleStep(step: number, time: number) {
    ...
    for (const clip of activeClips) {
      ...
      if (patternData.type === "drums") {
        const drumMap: DrumSound[] = ["clap", "hat", "snare", "kick"];
...
// Create empty grids
const createEmptyGrid = (rows: number) =>
  Array(rows).fill(null).map(() => Array(STEPS_PER_BAR).fill(false));

const DRUM_NAMES: { [key: number]: string } = { 0: "Clap", 1: "Hi-Hat", 2: "Snare", 3: "Kick" };
...
function DrumSequencer({ onSave, onExit, engine, patterns }: {
  ...
  const soundMap: DrumSound[] = ["clap", "hat", "snare", "kick"];
...
function MelodySequencer({ instrument, onSave, onExit, engine, patterns }: {
  ...
  const toggle = (gridRow: number, step: number) => {
    setGrid(g => {
      const newGrid = g.map(r => [...r]);
          ...
          {MELODY_ROLL_NOTES.map((note, noteIdx) => {
            ...
            return (
              <div key={note.name} className="flex items-center gap-px">
                ...
                {[...Array(STEPS_PER_BAR)].map((_, s) => {
                  const isActive = isClickable && grid[gridRow as number][s];
                  return (
                    <div
                      key={`${noteIdx}-${s}`}
                      onClick={() => isClickable && toggle(gridRow as number, s)}
...
function PianoSequencer({ onSave, onExit, engine, patterns }: {
  ...
  const notesDown = useRef<Map<number, number>>(new Map());
...
function LibraryPanel({ onSelectInstrument, patterns }: {
  ...
  const instruments: { id: InstrumentId, name: string, color: string }[] = [
  ...
  const onDragStart = (e: React.DragEvent, pattern: Pattern) => {
...
function TimelinePanel({ 
  ...
  setSelectedClipId: (id: string | null) => void, // NEW
  onCopy: () => void, // NEW
  onCut: () => void, // NEW
  onPaste: () => void, // NEW
  onDelete: () => void // NEW
}) {
  const lanes: InstrumentId[] = ["drums", "bass", "synth", "piano"];
  ...
  const onDrop = (e: React.DragEvent, lane: InstrumentId) => {
...
export default function LoopArranger() {
  ...
  const onSavePattern = (pattern: Pattern) => {
